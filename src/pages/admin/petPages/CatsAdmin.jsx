...
const CatsAdmin = () => {
  ...
  const currentPets = filteredCats.slice(indexOfFirstItem, indexOfLastItem);
  ...
  const deleteCat = async (id) => {
    try {
      await axios.delete(`http://localhost:5018/api/cats/${id}`, { withCredentials: true });
      const response = await axios.get('http://localhost:5018/api/cats', { withCredentials: true });
      setData(response.data);
