...
const BirdsAdmin = () => {
  ...
  const filteredBirds = data.filter(bird =>
  ...
  const currentPets = filteredBirds.slice(indexOfFirstItem, indexOfLastItem);
  ...
  const deleteBird = async (id) => {
    try {
      await axios.delete(`http://localhost:5018/api/birds/${id}`, { withCredentials: true });
      const response = await axios.get('http://localhost:5018/api/birds', { withCredentials: true });
      setData(response.data);
      ...
      <div className="ml-12 sm:ml-14 p-2">
        <div className="flex flex-col sm:flex-row items-center justify-between">
          <div className="flex items-center">
            <Link to="/dashboard/birds">
              <h1 className="text-4xl font-bold font-poppins text-primary-deep mr-4 text-right">Birds</h1>
            </Link>
            <div className="m-0">
          ...
          <tbody>
            {currentPets.map((pet, index) => (
              <tr key={index} className="bg-gray-100">
                ...
                <td className="border px-4 py-2 bg-primary-light text-center border-white">{pet.species}</td>
                <td className="border px-4 py-2 bg-primary-light text-center border-white">{pet.place_of_found}</td>
                <td className="border px-4 py-2 bg-white text-center">
                  ...
                  <button onClick={() => deleteBird(pet._id)} className="text-white bg-red-500 hover:bg-red-700 p-1 rounded">
                    <FaTrash />
                  </button>
                </td>
              </tr>
        ...
        <div className="flex justify-center mt-4">
          {[...Array(Math.ceil(filteredBirds.length / petPerPage)).keys()].map(number => (
            <button
              key={number}
